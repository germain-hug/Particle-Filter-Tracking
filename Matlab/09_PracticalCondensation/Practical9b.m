...
particles_old(:,3) = randn(numParticles,1)*1; % Randomly pick initial velocity
particles_old(:,4) = randn(numParticles,1)*1;
...
for( iTime = 1:22 )
    ...
    for (particleNum = 1:numParticles)
        ...
        noise_position = randn(1,2)*10;
        noise_velocity = randn(1,2)*1;
        index = samples_to_propagate(particleNum);

        % w_t+1 = w_t + v_t + noise_position
        %index = rand( numParticles, numDims_w);
        particles_new(particleNum,1:2) = particles_old(index,1:2) + ...
                                         particles_old(index,3:4) + noise_position;
        % v_t+1 = v_t + noise_velocity
        particles_new(particleNum,3:4) = particles_old(index,3:4) + noise_velocity;
        
         % Round the particles_new to simplify Likelihood evaluation.
        particles_new(particleNum,:)   = round(particles_new(particleNum,:));
    end;
    
    % Nb: What we do here is that we add random noise ie. we don't take
    % into account the actual movement between the frames. After adding
    % random velocity to the new particles we weight them by their
    % likelihood ie their similarity with the wheel template, and resample
    ...
    for (particleNum = 1:numParticles)
        ...
        if( inFrame )
            ...
            % im : image to search -> Here it's the current frame
            % template : the given template -> The wheel
