...
img = double(imread( 'abstract.png' )); % Will be used as the likelihood
MeasurementsComprehensive = img(:,:,1); % at stage 3 of Condensation Alg.
...
while(iTime < 10) % -> We converge to the likelihood (coherent, no
                       % temporal model)
iTime = iTime + 1;
...
xlabel(sprintf( 'Indices of all available samples, with larger blue circles for frequently re-sampled particles\n(Iteration %d)', iTime));
